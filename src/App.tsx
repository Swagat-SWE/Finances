...
function SpendingFlowChart({ title, accountList, data, transactions, range, onRangeChange, onExpand, onPointClick, expanded = false, combined = false }: { title: string; accountList: SpendingFlowAccount[]; data: SpendingFlowPoint[]; transactions: Transaction[]; range: SpendingFlowRange; onRangeChange: (range: SpendingFlowRange) => void; onExpand?: () => void; onPointClick?: (point: SpendingChartPoint) => void; expanded?: boolean; combined?: boolean }) {
  ...
  return <section className={`spending-flow-card ${combined ? 'spending-flow-card-combined' : ''}`}>
    <div className="spending-flow-card-header">
      ...
      <div className="spending-flow-card-actions"><div className="spending-flow-total" data-tour={combined ? 'spending-ytd' : undefined}><FilterDropdown label="Range" hideLabel compact value={range} options={spendingFlowRanges} onChange={value => onRangeChange(value as SpendingFlowRange)}/></div>{onExpand && !expanded && <button className="spending-flow-expand" data-tour={combined ? 'spending-expand' : undefined} type="button" aria-label={`Expand ${title} chart`} title="Expand chart" onClick={onExpand}><Maximize2 size={15}/></button>}</div>
    </div>
...
export default function App() {
  ...
  const [tourScope, setTourScope] = useState<'full' | 'current' | null>(null)
  ...
  // The import screen owns the first part of every tour. If navigation leaves
  // a dashboard step behind for a render (for example after choosing the
  // full-site tour from the import screen), normalize it immediately to the
  // upload step instead of showing a dashboard-only target such as Hide
  // numbers that does not exist on this page.
  useEffect(() => {
    if (view === 'import' && tourStep.startsWith('dashboard-')) setTourStep('import-dropzone')
  }, [view, tourStep])
  useEffect(() => {
  ...
  const skipImportTour = () => { setTourScope(null); setTourStep('idle') }
  const skipDashboardTour = () => { setTourScope(null); setTourStep('idle') }
  const beginTourIfNeeded = () => { setTourScope('full'); setTourStep('dashboard-import') }
  const startFullWebsiteTour = () => {
  ...
  const startCurrentPageTour = () => {
    const starts: Record<string, TourStep> = {
      Overview: 'dashboard-hide-numbers',
      Spending: 'dashboard-spending',
      Categories: 'dashboard-category-by-card',
      Merchants: 'dashboard-merchant-by-card',
      Statements: 'dashboard-statements-coverage',
    }
    setTourScope('current')
    setTourStep(starts[activeNav] ?? 'dashboard-hide-numbers')
  }
