...
export default function App() {
  ...
  const skipTour = () => { if (typeof window !== 'undefined') window.localStorage.setItem('finances.onboardingTour', 'completed'); setTourStep('idle') }
  const beginTourIfNeeded = () => { if (typeof window === 'undefined' || window.localStorage.getItem('finances.onboardingTour') !== 'completed') setTourStep('dashboard-import') }
  const finishTour = skipTour
  const replayTour = () => { setView('dashboard'); setTourStep('dashboard-import') }
  ...
  if (view === 'import') return <><ImportFlow accounts={accounts} categories={categories} importedStatements={importedStatements} tourStep={tourStep} onTourStep={setTourStep} onTourSkip={skipTour} onTourFinish={finishTour} onTourBack={replayTour} onTourReplay={replayTour} onBack={() => setView('dashboard')} onComplete={finishImport}/></>
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-is-collapsed' : ''}`}>
    ...
    <OnboardingTour step={tourStep} onSkip={skipTour}/>
  </div>
}
